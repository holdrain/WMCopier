...
def get_parser():
    ...
    parser.add_argument('--end_step', type=int, default=40,help='Step size for diffusion range')
    parser.add_argument('--M', type=int, default=100,help='M for refinement')
    parser.add_argument('--v', type=float, default=0.01,help='v for refinement')
    ...
    parser.add_argument('--fix', action='store_true',help='fix the last step')
    ...
    for dataset in ['diffusiondb','mscoco','celebahq','imagenet']:
        ...
        if args.no_forge:
            for x_0 in dataloader:
                ...
                x_0 = x_0 * 0.5 + 0.5
                for r_idx in range(x_0.shape[0]):
                    if args.save_sample:
                        save_count += 1
                        save_image(x_0[r_idx],os.path.join(save_dir,f"{save_count:05d}.png"),normalize=True)
            ...
            for x_0 in dataloader:
                ...
                rec_images,_ = sample_zp(x_0, pipeline, start_step, intermediate_latents,
                                args.num_inference_steps, args.device,likelihood=True, fix=args.fix, M=args.M,eta=args.eta,beta=args.beta,v=args.v)
                ...
                for r_idx in range(rec_images.shape[0]):
                    if args.method not in ['titan','treering']:
            ...
            print(f"Bit-Accuracy: {bitwise_accuracy_avg:.4f}, PSNR: {psnr_avg:.4f} FPR: {fp_sum/args.num_images}")
            ...
            with open(args.result_file, 'a') as f:
                ...
                f.write("\n[Metrics]\n")
                f.write(f"{dataset}: Bit-Accuracy={bitwise_accuracy_avg:.4f}, PSNR={psnr_avg:.4f}, FPR={fp_sum/args.num_images:.4f}\n")
                f.write("\n" + "="*60 + "\n\n")
